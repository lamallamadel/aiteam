...
# Encryption key for column-level encryption (AES-256)
vault kv put secret/atlasia encryption-key="$ENCRYPTION_KEY"
echo "✓ Stored encryption-key (AES-256 for database column encryption)"

# SSL/TLS configuration (placeholders - run gen-ssl-cert.sh first)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
KEYSTORE_PATH="file:${SCRIPT_DIR}/certs/keystore.p12"
vault kv put secret/atlasia ssl-keystore-path="$KEYSTORE_PATH"
vault kv put secret/atlasia ssl-keystore-password="changeit"
echo "✓ Stored SSL keystore configuration (run gen-ssl-cert.sh to generate certificates)"
...
echo "   ./gen-ssl-cert.sh"
echo ""
echo "2. Update SSL keystore path in Vault if needed:"
echo "   vault kv put secret/atlasia ssl-keystore-path=file:/absolute/path/to/keystore.p12"
echo ""
echo "3. Update OAuth2 and LLM API keys with real values:"
echo "   vault kv put secret/atlasia llm-api-key='your-actual-key'"
echo "   vault kv put secret/atlasia oauth2-github-client-id='your-github-client-id'"
echo ""
echo "4. Start the infrastructure:"
